...
public object PIDSourceY : PIDSource//the object that the PID controller checks while it is being run on the other thread
{
    ...
    override fun getPIDSourceType() : PIDSourceType = return m_pidSource;
    
    override fun pidGet(): Double = return (Drivetrain.rotatedYVelocity); // Runs this function whenever the pid loop is called and takes the value as the input into the pid loop
}
