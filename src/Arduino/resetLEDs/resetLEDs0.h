...
class resetLEDs0 {
  public:
    ...
    resetLEDs0(uint8_t Number_Of_LEDs_In_Strip);
	// sk6812<D, 0> choosePort(uint8_t portNumber);
    ...
    void holdAndClear(uint16_t on_time, uint16_t off_time);
	  ...
	  void color_chase(uint8_t Wait, uint8_t Legnth, uint8_t offset[], grbw Main);
